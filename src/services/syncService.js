...
class SyncService {
  ...
  async initializeFromDb() {
    try {
      ...
      if (config.isRunning) {
        console.log('Sync was active in previous session, attempting to restore...');
        ...
        if (authInfo && authInfo.username && authInfo.password) {
          ...
          setTimeout(() => {
            if (global.io) { // Make sure socket.io is available
              console.log('Restoring sync process with saved credentials');
              this.startSync(
                config.syncPeriod,
              ...
              console.log('Socket.io not available yet, cannot restore sync');
              dbService.updateSyncStatus(false);
            }
          }, 2000);
        } else {
          console.log('No saved credentials found, cannot restore sync');
          await dbService.updateSyncStatus(false);
        }
      } else {
        console.log('No active sync from previous session');
      }
    } catch (error) {
